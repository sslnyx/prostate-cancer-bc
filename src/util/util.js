...
const isHash = (url) => {
    if (url.split('#').length > 1) return true;
    return false
}
...
function titleize(slug) {
    ...
    const irrelevance = ['a', 'an', 'the', 'am']

    return words.map(function (word) {
        if (irrelevance.includes(word)) return word;
        return word.charAt(0).toUpperCase() + word.substring(1).toLowerCase();
    }).join(' ');
}

const slugToTitle = (pathname) => {
    const name = pathname.split("/").slice(1)[0]
    return titleize(name)
}

export { isInternal, toSnakeCase, cleanText, removeBaseUrl, checkCreditCard, getCircularReplacer, financial, toSnakeCase2, titleCase, isHash, slugToTitle }